...
def find_3_correct_entries(entries):
    for x in range(len(entries)):
        for y in range(len(entries)):
            #make sure not trying to add to itself
            if y != x:
                sum_a = entries[x] + entries[y]

                if sum_a < 2020:
                    #then possibility of third one
                    difference = 2020 - sum_a
                    if difference in entries:
                        return entries[x], entries[y], difference
...
correct_entries = find_2_correct_entries(expense_report)
print(correct_entries)

product = find_product(correct_entries)
print(product)


correct_3_entries = find_3_correct_entries(expense_report)
print(correct_3_entries)

product_3_entries = find_product(correct_3_entries)
print(product_3_entries)